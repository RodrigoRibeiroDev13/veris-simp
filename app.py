...
# --- CRIPTOGRAFIA DINÂMICA EM MEMÓRIA ---
# Evita o 'RecursionError' e 'Secrets does not support item assignment'
credentials_config = st.secrets["credentials"].to_dict()
for username in credentials_config["usernames"]:
    password_pura = credentials_config["usernames"][username]["password"]
    # Atualizado para a versão nova da biblioteca
    credentials_config["usernames"][username]["password"] = stauth.Hasher.hash(password_pura)
...
# Renderização da tela de login ajustada para a nova versão
authenticator.login(location="main")

# Captura de dados de estado oficial do Streamlit
...
if authentication_status == False:
    ...
    st.write("") # Espaçador técnico antes das abas
    ...
    with tab_historico:
        ...
        for h in historico:
            with st.expander(f"📋 Placa: {h['placa']} | {h['marca']} - R$ {h['valor_final_fixado']:,.2f}"):
                ...
                # Obtém o retorno do gerador de PDF
                pdf_output = CalculadoraAgregados.gerar_pdf_laudo(h)
                
                # Tratamento para garantir o formato de bytes exigido pelo download_button
